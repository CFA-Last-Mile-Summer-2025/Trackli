...
const SettingsPage: React.FC = () => {
  ...
  const loadSettingsData = async () => {
    try {
      // Fetch name
      const nameRes = await fetchWithAuth("http://localhost:3002/username");
      const nameData = await nameRes.json();
      setFullName(nameData.name || "");

      // Fetch email
      const emailRes = await fetchWithAuth("http://localhost:3002/useremail");
  ...
  const handleSave = async () => {
    await fetchWithAuth("http://localhost:3002/updateusername", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ newName: fullName }),
    });

    await fetchWithAuth("http://localhost:3002/updateemail", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
              ...
              {activeTab === "profile" ? (
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-gray-600 max-w-md mx-auto">
                    ...
                    This section will be available soon with advanced security
