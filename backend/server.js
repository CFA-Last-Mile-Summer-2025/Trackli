...
async function fetchDataAndSave() {
  try {
    ...
    for (const job of jobs) {
      let salary = null;
      if (job.salary_raw !== null && job.salary_raw.value !== undefined) {
        salary = {
          currency: job.salary_raw.currency || "USD",
          minValue: job.salary_raw.value.minValue || 0,
          maxValue: job.salary_raw.value.maxValue || 0,
          unitText: job.salary_raw.value.unitText || "HOUR",
        };
      }
      // Stuff to be inserted into db
      const listing = {
